...
import static github.bewantbe.audio_analyzer_for_android.R.drawable.stop;
...
public class WaveRecordFragment extends Fragment
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        layout = inflater.inflate(R.layout.wave_record, null);

        mSwitch = (Switch) layout.findViewById(R.id.train_mode);
        mSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
                    Log.d(TAG, "processClick: yes");
                    Toast.makeText(getActivity(), "开启训练模式", Toast.LENGTH_SHORT).show();
                } else Toast.makeText(getActivity(), "关闭训练模式", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        sampleRate = sharedPref.getInt("button_sample_rate", 44100);
        ...
        if (isAWeighting) {
            ((SelectorText) layout.findViewById(R.id.dbA)).nextValue();
        }
        boolean isSpam = sharedPref.getBoolean("spectrum_spectrogram_mode", true);
        if (!isSpam) {
            ((SelectorText) layout.findViewById(R.id.spectrum_spectrogram_mode)).nextValue();
        }
        ...
        ((Button) layout.findViewById(R.id.button_sample_rate)).setText(Integer.toString(sampleRate));
        ((Button) layout.findViewById(R.id.button_fftlen)).setText(Integer.toString(fftLen));
        ((Button) layout.findViewById(R.id.button_average)).setText(Integer.toString(nFFTAverage));
        ...
        graphView = (AnalyzeView) layout.findViewById(R.id.plot);
        ...
        visit((ViewGroup) graphView.getRootView(), new Visit() {
            ...
            public void exec(View view) {
                view.setOnLongClickListener(WaveRecordFragment.this);
                view.setOnClickListener(WaveRecordFragment.this);
//                ((TextView) view).setFreezesText(true);
        ...
        ((Button) layout.findViewById(R.id.button_sample_rate)).setWidth((int) (w + 4 * DPRatio));
        popupMenuFFTLen = popupMenuCreate(
                res.getStringArray(R.array.fft_len), R.id.button_fftlen);
        ((Button) layout.findViewById(R.id.button_fftlen)).setWidth((int) (w + 4 * DPRatio));
        popupMenuAverage = popupMenuCreate(
                res.getStringArray(R.array.fft_ave_num), R.id.button_average);
        ((Button) layout.findViewById(R.id.button_average)).setWidth((int) (w + 4 * DPRatio));

        mDetector = new GestureDetectorCompat(getActivity(), new AnalyzerGestureListener());

        TextView tv = (TextView) layout.findViewById(R.id.textview_cur);
        ...
        ((TextView) layout.findViewById(R.id.textview_cur)).setTextSize(fs / DPRatio);
        ((TextView) layout.findViewById(R.id.textview_peak)).setTextSize(fs / DPRatio);

//        if (bSaveWav) {
//            ((TextView) layout.findViewById(R.id.textview_rec)).setHeight((int) (19 * DPRatio));
//        } else {
//            ((TextView) layout.findViewById(R.id.textview_rec)).setHeight((int) (0 * DPRatio));
//        }

        circleButton = (CircleButton) layout.findViewById(R.id.button_recording);
...
//        samplingThread.stopRecording();
//    }
//
//    public static void startRecording(){
//        samplingThread.startRecording();
//    }
...
//    Debug.stopMethodTracing();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (i == R.id.info) {
            ...
            Intent settings = new Intent(getActivity().getBaseContext(), MyPreferences.class);
            ...
            return true;
        } else if (i == R.id.info_recoder) {
            Intent int_info_rec = new Intent(getActivity(), InfoRecActivity.class);
...
//      popupMenuAverage.showAsDropDown(view, 0, 0);
    ...
    private ArrayAdapter<String> popupMenuAdapter(String itemTagArray[]) {
        ArrayAdapter<String> adapter =
                new ArrayAdapter<String>(getActivity(), android.R.layout.simple_list_item_1, itemTagArray) {
                    @Override
...
//          Log.i(TAG, "  fly pixels x=" + shiftingPixelX + " y=" + shiftingPixelY);
                    ...
                    WaveRecordFragment.this.invalidateGraphView();
    ...
    private void switchMeasureAndScaleMode() {
        isMeasure = !isMeasure;
        SelectorText st = (SelectorText) layout.findViewById(R.id.graph_view_mode);
        st.performClick();
    }
...
//        Log.i(TAG, "scaleEvent(): xy=" + x + " " + y + "  wc = " + wc[0] + " " + wc[1]);
    ...
    public boolean processClick(View v) {
        ...
        String value = " ";
        int i = v.getId();
        if (i == R.id.button_recording) {
            Log.e(TAG, "processClick: " + saveRecord);
            if (!saveRecord)
            {
                saveRecord = true;
                circleButton.setImageDrawable(getResources().getDrawable(R.drawable.stop));
            }
            else {
                saveRecord = false;
                circleButton.setImageDrawable(getResources().getDrawable(R.drawable.tooth));
            }
            bSaveWav = saveRecord;
            SelectorText st = (SelectorText) layout.findViewById(R.id.run);
...
//                ((TextView) layout.findViewById(R.id.textview_rec)).setHeight((int) (19 * DPRatio));
            ...
            return false;
        } else if (i == R.id.graph_view_mode) {
            ...
            editor.commit();
            return false;
        } else if (i == R.id.spectrum_spectrogram_mode) {
    ...
    private void refreshCursorLabel() {
        ...
        ((TextView) layout.findViewById(R.id.textview_cur))
    ...
    private void refreshRMSLabel() {
        ...
        TextView tv = (TextView) layout.findViewById(R.id.textview_RMS);
    ...
    private void refreshPeakLabel() {
        ...
        TextView tv = (TextView) layout.findViewById(R.id.textview_peak);
    ...
    private void refreshRecTimeLable() {
        ...
        ((TextView) layout.findViewById(R.id.textview_rec))
    ...
    Runnable paddingInvalidateRunnable = new Runnable() {
        ...
        public void run() {
            if (idPaddingInvalidate) {
                ...
                WaveRecordFragment.this.invalidateGraphView(paddingViewMask);
            }
        }
    };
    ...
    public class Looper extends Thread {
        ...
        public Looper() {
            isPaused1 = ((SelectorText) layout.findViewById(R.id.run)).getText().toString().equals("stop");
...
//      FramesPerSecondCounter fpsCounter = new FramesPerSecondCounter("Looper::run()");
            ...
            if (bSaveWavLoop) {
                wavWriter.start(mSwitch.isChecked());
...
//          fpsCounter.inc();
            ...
            if (bSaveWavLoop) {
                ...
                wavWriter.stop();
                notifyWAVSaved(wavWriter.currentDir);
            }
        }
...
//                    toast.show();
            ...
            Intent intent = new Intent("recordDone");
        ...
        private void updateRec() {
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    // data will get out of synchronize here
                    WaveRecordFragment.this.invalidateGraphView(VIEW_MASK_RecTimeLable);
                }
            });
        }
